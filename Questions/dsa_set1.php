...
<body>
   <h2 style="text-align:center"> DATA STRUCTURES & ALGORITHMS Set 1 </h2>
   <div class="container" style="margin:100">
   
<form action="dsa_set1.php" method="post"> 
	 <?php 
	      $conn=mysql_connect("localhost","root","");
		  mysql_select_db("oes");
		  $i=1;
		// $cat_id=$_GET['cat'];
		  $query="select * from dsa where q_set='set-1'";
...
if(isset($_POST['submit'])){
 ...
 $conn=mysql_connect("localhost","root","");
 mysql_select_db("oes");
   $query1="select  * from dsa where q_set='set-1'";
  ...
  <div class="progress">
    <div class="progress-bar" role="progressbar" aria-valuenow="<?php echo $marks;?>" aria-valuemin="0" aria-valuemax="<?php echo $total;?>" style="width:$per%">
	  ...
	  $check_result="select * from result where user_email='$email2' AND cat_id='dsa' AND q_set='set-1'";
	$run=mysql_query($check_result);
	if(mysql_num_rows($run)>0){
	$result_query="	UPDATE result  SET correct_marks='$marks',total_marks='$total',date='$date' WHERE user_email='$email2' AND cat_id='dsa' AND q_set='set-1' ";
		$rub2=mysql_query($result_query);
		
	}
	 else{
	 $result_query="insert into result(user_email,cat_id,correct_marks,q_set,total_marks,date) values('$email2','dsa','$marks','set-1','$total','$date')";
